...
#define FASTBOOT_MAX_ENDPOINTS 9
...
{
	(0x16+2), STRING_DESCRIPTOR,
	'S', 0x0, 'M', 0x0, 'D', 0x0, 'K', 0x0, '6', 0x0,
	'4', 0x0, '4', 0x0, '0', 0x0, '-', 0x0, '0', 0x0,
	'1', 0x0
};
...
#elif defined(CONFIG_S5P6440)
        writel(0x0, S5P_OTG_PHYPWR);// Suspend sig dis; Analog & OTG blocks Power Up; 
        writel(0x2, S5P_OTG_PHYCLK);// Clk Sel: 12MHz; Ref Clk En; 48MHz Clk is available at all times except in Suspend mode; 
#elif defined(CONFIG_S5P6450)
...
#elif defined(CONFIG_S5P6440)
        writel(readl(S5P_OTG_PHYPWR)|(0x3<<3), S5P_OTG_PHYPWR);
        OTHERS_REG &= ~(1<<16);// Misc Control Reg. in PM; USB Sig Mask Dis
#elif defined(CONFIG_S5P6450)
	writel(readl(S5P_OTG_PHYPWR)|(0x18), S5P_OTG_PHYPWR);
#else
#error "* CFG_ERROR : you have to select C100 or C110 for Android Fastboot"
...
#elif defined(CONFIG_S5PC100)
	dnCS = *(u16 *)cs_end;
#elif defined(CONFIG_S5P6440) || defined(CONFIG_S5P6450)
        dnCS = (u16)((cs_end[1]<<8) + cs_end[0]);
#else
#error "* CFG_ERROR : you have to select C100 or C110 for Android Fastboot"
#endif
...
void fboot_usb_set_all_outep_nak(void)
{
	...
	DBG0("\n");
	for(i=0;i<FASTBOOT_MAX_ENDPOINTS;i++)
	{
...
int fastboot_init(struct cmd_fastboot_interface *interface)
{
	...
	device_strings[DEVICE_STRING_PRODUCT_INDEX]       = "smdkv210";
        ...
        device_strings[DEVICE_STRING_PRODUCT_INDEX]       = "smdk6440";
#elif defined(CONFIG_SMDK6450)  
        device_strings[DEVICE_STRING_PRODUCT_INDEX]       = "smdk6450";
#else
