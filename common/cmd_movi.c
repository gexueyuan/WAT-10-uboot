...
int do_movi(cmd_tbl_t * cmdtp, int flag, int argc, char *argv[])
{
	...
	if (strcmp(cmd, "init") == 0) {
		...
		if (argc == 4 || (strcmp(argv[2], "rootfs") == 0))addr = (ulong) simple_strtoul(argv[3], NULL, 16);
		else if (argc == 5)
...
int do_movi(cmd_tbl_t * cmdtp, int flag, int argc, char *argv[])
{
	...
	case 'i':
		...
		run_command("mmcinfo", MMC_BOOT_DEVICE);
		return 1;
	case 'r':
		rw = 0;	/* read case */
		break;
...
#else
	mmc = find_mmc_device(MMC_BOOT_DEVICE);
#endif
	...
	#else
		sprintf(run_cmd, "mmc %s %d 0x%lx 0x%lx 0x%lx",
			rw ? "write" : "read",MMC_BOOT_DEVICE, addr, start_blk, blkcnt);
	#endif
		run_command(run_cmd, 0);
